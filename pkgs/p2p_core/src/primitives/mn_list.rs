...
impl BaseCodec for MnListDiffPayload {
  fn decode(data: &mut &[u8]) -> Result<Self, DecodeError> {
    ...
    for _ in 0..mn_count {
      mn_list.push(BaseCodec::decode(data)?);
    }
    ...
    for _ in 0..nq_count {
      let commitment: Commitment = BaseCodec::decode(data).map_err(|_| DecodeError::Eof {
        needed: 1,
        remaining: 0,
      })?;
