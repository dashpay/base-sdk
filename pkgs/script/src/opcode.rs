...
enum_map! {
  /// Script opcode.
  ///
  /// Every named variant corresponds to exactly one byte value on the wire.
  /// Bytes `0x01..=0x4b` are direct data pushes (the byte *is* the push length);
  /// use [`Opcode::is_direct_push`] to test for them before mapping a byte.
  ///
  /// Any byte without a named variant, direct pushes included, decodes to
  /// `Unknown(byte)` and re-encodes to that same byte. This differs from
  /// `OP_INVALIDOPCODE` (`0xff`), which is a real opcode the consensus rules
  /// define; do not treat the two as interchangeable.
