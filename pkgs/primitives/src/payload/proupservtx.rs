...
use dash_types::{type_id::TypeId, Checkable, Numeric};
...
mod tests {
  ...
  fn corpus_proupservtx() {
    let corpus = Corpus::open(env!("CARGO_MANIFEST_DIR"), "proupservtx");
    let items = corpus.entries::<ProUpServTx>("proupservtx", check_sptx);
