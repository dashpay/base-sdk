...
| ---- | ------- | ---------- | ---------- |
| [`lint_cargo.py`](./lint/lint_cargo.py) | Enforce MSRV across Rust build dependency graph, deny dependencies per [`deny.toml`](../deny.toml), check/format TOML files against [`.taplo.toml`](../.taplo.toml) | `check` , `apply`, `apply-all` | `cargo` (MSRV enforcement), `cargo-deny` (deny dependencies), `taplo` (TOML formatting) |
